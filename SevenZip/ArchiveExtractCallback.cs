                                                   ...
                                                   IDisposable
    {
                ...
                if (!_fileIndex.HasValue)
                {
                    ...
                    if (_actualIndexes == null || _actualIndexes.Contains(index))
                    {
                        ...
                        if (!NativeMethods.SafeCast(data, false))
                        {
                            ...
                            try
                            {
                                _fileStream = new OutStreamWrapper(File_Create(fileName), fileName, time, true);
                            }
                            catch (Exception e)
                            {
                                if (e is FileNotFoundException)
                                {
                                    AddException(
                                        new IOException("The file \"" + fileName +
                                                        "\" was not extracted due to the File_Create fail."));
                                }
                                else
                                {
                                    AddException(e);
		...
		public static Exception GetException(int errorCode, string path)
		{
			switch (errorCode)
			{
				...
				case 15:
					return new DriveNotFoundException(path);
			}

			return new IOException("Error with file " + path, errorCode);
		}
